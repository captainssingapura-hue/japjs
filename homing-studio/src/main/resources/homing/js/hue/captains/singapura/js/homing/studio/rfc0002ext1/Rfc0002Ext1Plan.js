// =============================================================================
// Homing studio — RFC 0002-ext1 plan (Utility-First Composition + Two-Layer Semantic Tokens)
...
function appMain(rootElement) {
    ...
    var shellHtml = ''
        ...
        + '    <span class="' + cn(st_crumb_sep) + '">/</span>'
        + '    <span class="' + cn(st_crumb) + '">RFC 0002-ext1 Plan</span>'
        + '  </div>'
        + '</div>'

        + '<div class="' + cn(st_main) + '">'
        + '  <div id="planRoot"><div class="' + cn(st_loading) + '">Loading RFC 0002-ext1 plan…</div></div>'
        + '</div>'
    ...
    function renderPlan(data) {
        var html = ''
            + '<div class="' + cn(st_kicker) + '">RFC 0002-ext1</div>'
            + '<h1 class="' + cn(st_title) + '">Utility-First + Semantic Tokens</h1>'
            + '<p class="' + cn(st_subtitle) + '">Source of truth: <code>Rfc0002Ext1Steps.java</code>. Edit, recompile, refresh — phases and decisions update live. Companion document: <a ' + href.toAttr(nav.DocReader({path: data.rfcDoc})) + ' style="color:var(--st-amber-dk);">RFC 0002-ext1 (full design)</a>.</p>';
        ...
        if (data.decisions && data.decisions.length > 0) {
            html += '<div class="' + cn(st_section) + '">'
                  ...
                  + '  <p style="font-size:14px;color:var(--st-gray-mid);margin:0 0 14px 0;font-style:italic;">Resolve these during execution. Edit <code>Rfc0002Ext1Steps.DECISIONS</code> to mark resolved with a chosen value.</p>';
        ...
        for (var i = 0; i < data.phases.length; i++) {
            ...
            html += '<a class="' + cn(st_step_card) + '" ' + href.toAttr(nav.Rfc0002Ext1Step({phase: p.id})) + '>'
        ...
        html += '<div class="' + cn(st_footer) + '">'
              ...
              + '  <br/>Raw JSON endpoint: <code>/rfc0002ext1-data</code>.'
              + '</div>';
