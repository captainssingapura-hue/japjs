// =============================================================================
// Homing studio — RFC 0002 phase detail view
// =============================================================================
...
function appMain(rootElement) {
    ...
    var shellHtml = ''
        ...
        + '    <span class="' + cn(st_crumb_sep) + '">/</span>'
        + '    <a class="' + cn(st_crumb) + '" ' + href.toAttr(nav.Rfc0002Plan()) + '>RFC 0002 Plan</a>'
        + '    <span class="' + cn(st_crumb_sep) + '">/</span>'
    ...
    fetch("/rfc0002-data?phase=" + encodeURIComponent(phaseId))
        .then(function(r) {
    ...
    function renderPhase(p) {
        ...
        var html = ''
            + '<div class="' + cn(st_kicker) + '">RFC 0002 · Phase ' + escape(p.id) + '</div>'
        ...
        if (!isNaN(idNum)) {
            ...
            html += '<div style="display:flex;justify-content:space-between;margin-top:24px;font-size:13px;">'
                  + '  <span>'
                  +    (prevId ? '<a ' + href.toAttr(nav.Rfc0002Step({phase: prevId})) + ' style="color:var(--st-amber-dk);text-decoration:underline;">← Phase ' + prevId + '</a>' : '')
                  + '  </span>'
                  + '  <a ' + href.toAttr(nav.Rfc0002Plan()) + ' style="color:var(--st-gray-mid);text-decoration:underline;">All phases</a>'
                  + '  <span>'
                  +    (nextId ? '<a ' + href.toAttr(nav.Rfc0002Step({phase: nextId})) + ' style="color:var(--st-amber-dk);text-decoration:underline;">Phase ' + nextId + ' →</a>' : '')
                  + '  </span>'
                  + '</div>';
        }
