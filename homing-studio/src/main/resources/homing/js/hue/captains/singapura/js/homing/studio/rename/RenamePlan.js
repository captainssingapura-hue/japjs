// =============================================================================
// Homing studio — rename plan (Homing → Homing)
// Source of truth: RenameSteps.java. Edit, recompile, refresh.
// =============================================================================
...
function appMain(rootElement) {
    ...
    var shellHtml = ''
        ...
        + '    <span class="' + cn(st_crumb_sep) + '">/</span>'
        + '    <span class="' + cn(st_crumb) + '">Rename Plan</span>'
        + '  </div>'
        + '</div>'

        + '<div class="' + cn(st_main) + '">'
        + '  <div id="planRoot"><div class="' + cn(st_loading) + '">Loading rename plan…</div></div>'
        + '</div>'
    ...
    function renderPlan(data) {
        var html = ''
            + '<div class="' + cn(st_kicker) + '">Project rename</div>'
            + '<h1 class="' + cn(st_title) + '">' + escape(data.oldName) + ' → ' + escape(data.newName) + '</h1>'
            + '<p class="' + cn(st_subtitle) + '">Source of truth: <code>RenameSteps.java</code>. Edit, recompile, refresh — phases and decisions update live. Companion document: <a ' + href.toAttr(nav.DocReader({path: data.executionDoc})) + ' style="color:var(--st-amber-dk);">execution plan (full prose)</a>.</p>';
        ...
        if (data.decisions && data.decisions.length > 0) {
            html += '<div class="' + cn(st_section) + '">'
                  ...
                  + '  <p style="font-size:14px;color:var(--st-gray-mid);margin:0 0 14px 0;font-style:italic;">Resolve these before executing Phase 1. Edit <code>RenameSteps.DECISIONS</code> to mark resolved with a chosen value.</p>';
        ...
        for (var i = 0; i < data.phases.length; i++) {
            ...
            html += '<a class="' + cn(st_step_card) + '" ' + href.toAttr(nav.RenameStep({phase: p.id})) + '>'
        ...
        html += '<div class="' + cn(st_footer) + '">'
              + '  References: <a ' + href.toAttr(nav.DocReader({path: data.executionDoc})) + '>Execution Plan (prose)</a>'
              + '  &nbsp;·&nbsp; <a ' + href.toAttr(nav.DocReader({path: data.dossierDoc})) + '>Rename Dossier (decision context)</a>'
              + '  <br/>Raw JSON endpoint: <code>/rename-data</code>.'
              + '</div>';
