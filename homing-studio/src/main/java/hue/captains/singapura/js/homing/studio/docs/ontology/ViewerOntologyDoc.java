...
public record ViewerOntologyDoc() implements ClasspathMarkdownDoc {
    ...
    @Override public String summary() { return "A Viewer is the framework type that renders content of one specific kind. Stateless, read-only, registered binding between a content kind and an AppModule that renders Docs of that kind. Ten axioms — identity by kind, kind exclusivity (one kind, one Viewer), Doc routing via kind, one Viewer many Docs, canonical URL composition, read-only rendering, stateless across users, registration as activation, stateless functional object. Realised by DocReader, PlanAppHost, StudioGraphInspector. Definition and axioms only — operational guidance lives in Doctrines."; }
