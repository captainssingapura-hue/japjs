...
/** Adapter — exposes {@link Rfc0004Ext1Steps} as a {@link Plan}. */
public final class Rfc0004Ext1PlanData implements Plan {

    public static final Rfc0004Ext1PlanData INSTANCE = new Rfc0004Ext1PlanData();

    private Rfc0004Ext1PlanData() {}

    @Override public String kicker()        { return "RFC 0004-ext1"; }
    @Override public String name()          { return "Managed Markdown References"; }
    @Override public String subtitle() {
        return "Source of truth: Rfc0004Ext1Steps.java. Edit, recompile, refresh — phases, decisions, and metrics update live.";
    }
    @Override public int    totalProgress() { return Rfc0004Ext1Steps.totalProgressPercent(); }
    @Override public int    openDecisions() { return Rfc0004Ext1Steps.openDecisionsCount(); }
    @Override public String executionDoc()  { return Rfc0004Ext1Steps.RFC_DOC; }
    @Override public String dossierDoc()    { return null; }

    @Override
    public List<Phase> phases() {
        return Rfc0004Ext1Steps.PHASES.stream().map(Rfc0004Ext1PlanData::adaptPhase).toList();
    }

    @Override
    public List<Decision> decisions() {
        return Rfc0004Ext1Steps.DECISIONS.stream().map(Rfc0004Ext1PlanData::adaptDecision).toList();
    }
    ...
    private static Phase adaptPhase(Rfc0004Ext1Steps.Phase p) {
        return new Phase(
                p.id(),
                p.label(),
                p.summary(),
    ...
    private static Decision adaptDecision(Rfc0004Ext1Steps.Decision d) {
        return new Decision(
                d.id(),
                d.question(),
                d.recommendation(),
    ...
    private static PhaseStatus adaptStatus(Rfc0004Ext1Steps.Status s) {
        return switch (s) {
    ...
    private static DecisionStatus adaptDecisionStatus(Rfc0004Ext1Steps.DecisionStatus s) {
        return switch (s) {
