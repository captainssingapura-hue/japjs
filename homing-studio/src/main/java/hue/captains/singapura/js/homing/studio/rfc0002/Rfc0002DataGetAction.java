...
/**
 * GET /rfc0002-data            → JSON of decisions + all phases
 * GET /rfc0002-data?phase=03   → JSON of one phase
 *
 * <p>Serves the live state of {@link Rfc0002Steps}. Edit that file, recompile,
 * refresh — the new state appears in the studio.</p>
 */
public class Rfc0002DataGetAction
        implements GetAction<RoutingContext, Rfc0002DataGetAction.Query, EmptyParam.NoHeaders, Rfc0002DataGetAction.Json> {
    ...
    public CompletableFuture<Json> execute(Query query, EmptyParam.NoHeaders headers) {
        ...
        var phase = Rfc0002Steps.phaseById(query.phaseId());
        if (phase == null) {
    ...
    private static String serializeAll() {
        ...
        sb.append("{\"rfcDoc\":").append(jstr(Rfc0002Steps.RFC_DOC)).append(",")
          .append("\"totalProgress\":").append(Rfc0002Steps.totalProgressPercent()).append(",")
          .append("\"openDecisions\":").append(Rfc0002Steps.openDecisionsCount()).append(",")
          .append("\"decisions\":[");
        boolean firstD = true;
        for (var d : Rfc0002Steps.DECISIONS) {
            if (!firstD) sb.append(","); firstD = false;
            sb.append(serializeDecision(d));
        }
        sb.append("],\"phases\":[");
        boolean firstP = true;
        for (var p : Rfc0002Steps.PHASES) {
    ...
    private static String serializeDecision(Rfc0002Steps.Decision d) {
        return "{"
             + "\"id\":" + jstr(d.id()) + ","
    ...
    private static String serializePhase(Rfc0002Steps.Phase p) {
        StringBuilder sb = new StringBuilder();
        sb.append("{")
