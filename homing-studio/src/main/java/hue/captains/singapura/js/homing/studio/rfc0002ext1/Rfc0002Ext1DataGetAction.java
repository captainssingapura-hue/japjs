...
/**
 * GET /rfc0002ext1-data            → JSON of decisions + all phases
 * GET /rfc0002ext1-data?phase=01   → JSON of one phase
 *
 * <p>Serves the live state of {@link Rfc0002Ext1Steps}. Edit that file, recompile,
 * refresh — the new state appears in the studio.</p>
 */
public class Rfc0002Ext1DataGetAction
        implements GetAction<RoutingContext, Rfc0002Ext1DataGetAction.Query, EmptyParam.NoHeaders, Rfc0002Ext1DataGetAction.Json> {
    ...
    public CompletableFuture<Json> execute(Query query, EmptyParam.NoHeaders headers) {
        ...
        var phase = Rfc0002Ext1Steps.phaseById(query.phaseId());
        if (phase == null) {
    ...
    private static String serializeAll() {
        ...
        sb.append("{\"rfcDoc\":").append(jstr(Rfc0002Ext1Steps.RFC_DOC)).append(",")
          .append("\"totalProgress\":").append(Rfc0002Ext1Steps.totalProgressPercent()).append(",")
          .append("\"openDecisions\":").append(Rfc0002Ext1Steps.openDecisionsCount()).append(",")
          .append("\"decisions\":[");
        boolean firstD = true;
        for (var d : Rfc0002Ext1Steps.DECISIONS) {
            if (!firstD) sb.append(","); firstD = false;
            sb.append(serializeDecision(d));
        }
        sb.append("],\"phases\":[");
        boolean firstP = true;
        for (var p : Rfc0002Ext1Steps.PHASES) {
    ...
    private static String serializeDecision(Rfc0002Ext1Steps.Decision d) {
        return "{"
             + "\"id\":" + jstr(d.id()) + ","
    ...
    private static String serializePhase(Rfc0002Ext1Steps.Phase p) {
        StringBuilder sb = new StringBuilder();
        sb.append("{")
